...
#if defined(EXHAUSTIVE_TEST_ORDER)
{
     ...
     *已经计算出了R点，以及他所声称的x坐标mod n在xr中，因此从R点中提取x坐标，需要mod p求逆，计算mod n的余数，并与xr比较
     *        xr == X(pr) mod n
 ...
 * @param {secp256k1_scalar} *sigr  签名值之一r  R点的scalar类型的x坐标
 * @param {secp256k1_scalar} *sigs 签名值之一s
 * @param {secp256k1_scalar} *seckey 签名方的私钥
 * @param {secp256k1_scalar} *message 签名消息
 * @param {secp256k1_scalar} *nonce  随机生成的数r
 * @param {int} *recid
 * @return {*}
 */
static int secp256k1_ecdsa_sig_sign(const secp256k1_ecmult_gen_context *ctx, secp256k1_scalar *sigr, secp256k1_scalar *sigs, const secp256k1_scalar *seckey, const secp256k1_scalar *message, const secp256k1_scalar *nonce, int *recid) {
    ...
    /* 把点R的坐标x的字符形式转为了scalar类型sigr 此为签名值之一 */
