...
//     secp256k1_scalar nonce1,seckey1,seckey2,DLCV;
//     secp256k1_ge gama;
...
//     int ret = Generate_key_pairs_and_gamma_values(&CTX->ecmult_gen_ctx,&nonce1,&seckey1,&DLCV,&seckey2,&gama);
//     if(ret == 1){
//         printf("密钥和gama值生成成功！\n");
//     }
...
/* 解析json数据成报告结构体 通过测试*/
void Json_to_Report_structure(cJSON *cjson,EC_Equipment_Certification_Report *data){
    /* 设备信息 */
...
void Aggregate_verification()
{
    ...
    FILE *fp = NULL;
    // 打开文件
    fp = fopen("./data/runtime_RV_EC5.txt", "a");
    // 向文件中写入数据
    fprintf(fp, "验证节点数：%d\t实际验证：%d\t\t证书验证时间:%f\t\t聚合验证时间:%f\t\t存储量：%ld\n", COUNT, Aggindex + datalistnum,certTime, aggduration,memory_capacity+((2717+200)*(datalistnum+Aggindex)));
    fclose(fp);
    /* 结束之后释放内存 */
    free(outpub);
    free(A);
