...
/* For accelerating the computation of a*G: 加速计算aG
...
static void secp256k1_ecmult_gen_blind(secp256k1_ecmult_gen_context *ctx, const unsigned char *seed32) {
    ...
    if (seed32 == NULL) {
        ...
        /*secp256k1_gej_set_ge(&ctx->initial, &max);*/
        secp256k1_gej_neg(&ctx->initial, &ctx->initial);
        secp256k1_scalar_set_int(&ctx->blind, 1);
        return;
    }
    /* The prior blinding value (if not reset) is chained forward by including it in the hash. */
    /*先前的blinding（如果未重置）通过将其包含在哈希中而向前链接。*/
     ...
     */
    /**
     * 使用CSPRNG允许无故障接口、避免需要大量随机数据，
        并防止软弱或敌对的种子。这是一个更简单、更安全的接口，而不是直接要求调用方提供盲值，并期望它们在失败时重试。
    */
    ...
    /*将投影随机化，以防御乘数侧通道。 在我们自己调用下面的secp256k1_ecmult_gen之前执行此操作。*/
