...
void Device_nlayer_startup_test(void)
{
    ...
    /* 先验证证书 */
    Verification_certificate(10001);
...
EC_Equipment_Certification_Report datalist[100];
/* datalist的长度 聚合时需要使用 */
int datalistnum = 0;
/* datalist中对应的节点的ID */
int datalistID[100];
...
void *Judgment_aggregation()
{
    ...
    while (1)
    {
        ...
        if ((rece_report_num == childNum && !hasAGG) || (times == 0 && !hasAGG))
        {
            ...
            FILE *fp2 = NULL;
            // 打开文件
            fp = fopen("./data/runtime_SN_EC5.txt", "a");
            fp2 = fopen("temp.txt", "w");
            // 向文件中写入数据
            if (COUNT == 5)
            {
                fprintf(fp, "验证节点数：%d\t实际聚合：%d\t处理数据时间:%f\t\t启动时间：%f\t\t聚合时间:%f\n", COUNT, datalistnum + Aggindex, dataProcessTime, suTime, aggtime);
            }
            else if (COUNT == 40)
            {
                // 报告中没有包含最后一层证书 所以要额外加 聚合报告中不含证书链 否则数据太大 影响传输
                fprintf(fp, "验证节点数：%d\t实际聚合：%d\t处理数据时间:%f\t\t聚合时间:%f\t\t存储量:%ld\n", COUNT, datalistnum + Aggindex, dataProcessTime, aggtime, memory_capacity + ((200) * (datalistnum + Aggindex)) + Aggindex * (2717-128) + 591 * datalistnum);
            }
            else
            {
                fprintf(fp, "验证节点数：%d\t实际聚合：%d\t处理数据时间:%f\t\t聚合时间:%f\n", COUNT, datalistnum + Aggindex, dataProcessTime, aggtime);
            }
...
int main(int argc, char **argv)
{
    ...
    gossip_seed_node();
    // Device_nlayer_startup_test();
    // Convert_the_certificate_chain_to_json_data();
